...
srun --jobid $SLURM_JOBID bash -c 'python -m torch.distributed.run \
 ...
 training/train.py \
  --pretrained_checkpoint path/to/cronusvla_modified_ckpt_post-train/step2/checkpoints/reshape_embedding_step2.pt \
  --vla.type prism-dinosiglip-224px+oxe+diffusion \
  --vla.data_mix libero_goal_no_noops \
  --vla.view_sequence_len 2 \
  ...
  --run_root_dir ./outputs/LIBERO_goal \
  --run_id LIBERO_goal \
  --image_aug True \
