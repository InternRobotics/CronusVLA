...
from experiments.Libero.robot.openvla_utils import (
    get_vla,
    get_vla_action,
)
from experiments.Libero.robot.cronus_utils import (
    CronusServer
)
...
def get_model(cfg, wrap_diffusion_policy_for_droid=False):
    ...
    if cfg.model_family == "openvla":
        ...
        cronus_server = CronusServer(cfg)
...
def get_image_resize_size(cfg):
    ...
    """
    if cfg.model_family == "openvla" or cfg.model_family == "cronus":
        resize_size = 224
    else:
        raise ValueError("Unexpected `model_family` found in config.")
    return resize_size
