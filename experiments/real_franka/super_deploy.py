        ...
        if self.action_ensemble:
            ...
            if self.action_chunking_window is not None:
                ...
                for i in range(0, self.action_chunking_window):
                    unnormed_actions[i][6] = unnormed_actions[i][6] >= 0.5
...
parser.add_argument("--saved_model_path", type=str, default="./checkpoints/step-003500-epoch-18-loss=0.0223.pt")
parser.add_argument("--unnorm_key", type=str, default="grasp_from_rack")
...
def inference():
    try:
        ...
        try:
            input_query = json.load(json_file.stream)
        except json.JSONDecodeError as e:
            return jsonify({"error": f"Error loading JSON: {str(e)}"}), 400

        print("Input query:", input_query)

        answer = inferencer.step(image, **input_query)
