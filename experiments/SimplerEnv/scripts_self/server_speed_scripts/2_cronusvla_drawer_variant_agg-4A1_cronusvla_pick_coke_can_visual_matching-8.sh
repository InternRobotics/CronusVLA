...
CUDA_VISIBLE_DEVICES=${gpu_id} python simpler_env/main_inference_server.py --host $AVAILABLE_HOST --port $AVAILABLE_PORT --policy-model cronusvla --ckpt-path $1 --env-name None &
SERVER_PID=$!
...
EvalSim() {
  ...
  CUDA_VISIBLE_DEVICES=${gpu_id} python simpler_env/main_inference_client.py --host $AVAILABLE_HOST --port $AVAILABLE_PORT --policy-model cronusvla --ckpt-path ${ckpt_path} \
...
do CUDA_VISIBLE_DEVICES=${gpu_id} python simpler_env/main_inference_client.py --host $AVAILABLE_HOST --port $AVAILABLE_PORT --policy-model cronusvla --ckpt-path ${ckpt_path} \
  --robot google_robot_static \
