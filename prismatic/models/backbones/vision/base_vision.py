...
class WrapSequenceImageTransform(ImageTransform):
    ...
    def __call__(self, img: Union[Image, List[Image]], **kwargs: str) -> Union[torch.Tensor, Dict[str, torch.Tensor]]:
        assert isinstance(img, list) and len(img) == self.sequence_len, f"{len(img)}: {self.sequence_len}"
...
class VisionBackbone(nn.Module, ABC):
    def __init__(
        ...
        self.view_sequence_len: int = view_sequence_len
    ...
    def get_image_transform(self) -> ImageTransform:
        if self.view_sequence_len == 1:
            return self.image_transform
        else:
            return WrapSequenceImageTransform(self.image_transform, sequence_len=self.view_sequence_len)
...
class TimmViTBackbone(VisionBackbone, ABC):
    ...
    def forward(self, pixel_values: Union[torch.Tensor, Dict[str, torch.Tensor]]) -> torch.Tensor:
        ...
        if self.view_sequence_len == 1:
            ...
            return compute_sequence_patches({"img": pixel_values}, {"img": self.featurizer}, self.view_sequence_len)[
                "img"
            ]
    ...
    @property
    def num_patches(self) -> int:
        return self.featurizer.patch_embed.num_patches * self.view_sequence_len
