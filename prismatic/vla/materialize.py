...
def get_vla_dataset_and_collator(
    ...
    if isinstance(image_transform, WrapSequenceImageTransform):
        if use_wrist_image:
            ...
            assert image_transform.sequence_len == 2, "With wrist images, image transform must expect 2 images!"
        else:
            assert image_transform.sequence_len == 1

    batch_transform = RLDSBatchTransform(
        action_tokenizer, tokenizer, image_transform, prompt_builder_fn, predict_stop_token=predict_stop_token,use_wrist_image=use_wrist_image,
    )
