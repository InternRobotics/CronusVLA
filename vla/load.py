...
def load(
    ...
    view_sequence_len: Optional[int] = None,
) -> PrismaticVLM:
    ...
    if view_sequence_len is None:
        if hasattr(model_cfg, "view_sequence_len"):
            view_sequence_len = model_cfg.view_sequence_len
        else:
            view_sequence_len = 1
...
def load_vla(
    ...
    **kwargs,
    ...
    """Loads a pretrained CronusVLA from either local disk or the HuggingFace Hub."""
    ...
    if os.path.isfile(model_id_or_path):
        ...
        assert (checkpoint_pt.suffix == ".pt") and ("checkpoints" in checkpoint_pt.parent.name), "Invalid checkpoint!"
    ...
    if view_sequence_len is None:
        if "view_sequence_len" in vla_cfg:
            view_sequence_len = vla_cfg["view_sequence_len"]
        else:
            view_sequence_len = 1
    else:
        if "view_sequence_len" in vla_cfg:
            assert view_sequence_len == vla_cfg["view_sequence_len"]
        else:
            vla_cfg["view_sequence_len"] = view_sequence_len
    print(f'!!!!!!!!!! view_sequence_len = {view_sequence_len}')
    # Load Vision Backbone
    overwatch.info(f"Loading Vision Backbone [bold]{model_cfg.vision_backbone_id}[/]")
    vision_backbone, image_transform = get_vision_backbone_and_transform(
        model_cfg.vision_backbone_id,
    ...
    if "past_action_window_size" in v_cfg.keys():
        if "past_action_window_size" in kwargs.keys():
            assert kwargs["past_action_window_size"] == v_cfg["past_action_window_size"]
        kwargs["past_action_window_size"] = v_cfg["past_action_window_size"]
    else:
        if "past_action_window_size" not in kwargs.keys():
            raise ValueError("missing parameter: past_action_window_size!")

    # Load VLM using `from_pretrained` (clobbers HF syntax... eventually should reconcile)
    overwatch.info(f"Loading VLA [bold blue]{model_cfg.model_id}[/] from Checkpoint")

    vla = CronusVLA.from_pretrained(
        checkpoint_pt,
        model_cfg.model_id,
        vision_backbone,
        llm_backbone,
