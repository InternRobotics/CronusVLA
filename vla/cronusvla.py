...
from vla.decoder.decoder import ActionModel
from vla.decoder.models import DiT
...
class CronusVLA(nn.Module):
    ...
    def forward(
        ...
        ###############  Feature Chunking  ###############
        cognition_features_detached = cognition_features[:,:-1,:].detach() # Stop gradient
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # Initialize CronusVLA
        cronusvla = CronusVLA(vlm,
        ...
        if "action_model" in model_state_dict:
            cronusvla.action_model.load_state_dict(model_state_dict["action_model"])
            if "ema_diffusion" in model_state_dict and use_ema:
                cronusvla.ema_diffusion.load_state_dict(model_state_dict["ema_diffusion"])
            elif use_ema:
                cronusvla.ema_diffusion.load_state_dict(model_state_dict["action_model"])
        else:
    ...
    def predict_action(
        ...
        **kwargs: str
        ...
        """
        ###############  Data Process  ###############
        image_transform, tokenizer = self.vlm.vision_backbone.get_image_transform(), self.vlm.llm_backbone.tokenizer
